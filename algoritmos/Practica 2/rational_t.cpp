...
inline int Rational::get_num() const{ // Retorna el valor del atributo
  return numerador_;
}

inline int Rational::get_den() const{ // Retorna el valor del atributo
  return denominador_;
}
...
inline double Rational::value() const{  // Método que retorna el valor de la división entre el numerador y el denominador
  return double(get_num()) / get_den();
}

Rational Rational::opposite() const{  // Función que devuelve el valor en negativo del obejeto de clase Rational
  return Rational((-1)*get_num(), get_den());
}

Rational Rational::reciprocal() const{ // Función que devuelve un valoe de clase rational en el que se intercambian el numerador y el denominador
  return Rational(get_den(), get_num());
}

// comparaciones
bool Rational::is_equal(const Rational& r, const double precision) const{ // Método que que te dice si son iguales
  return fabs(value() - r.value()) < precision;
}

bool Rational::is_greater(const Rational& r, const double precision) const{ // Método que que te dice si y es mayor que x
  return (value() - r.value()) > precision;
}

bool Rational::is_less(const Rational& r, const double precision) const{ // Método que que te dice si y es menor que la x
  return r.is_greater(value() - r.value()) > precision;
}

// operaciones
Rational Rational::add(const Rational& r) const{ // Método que devuelve la suma de a + b en forma de clase Rational
  return Rational((get_num() * r.get_den()) + (get_den() * r.get_num()), get_den() * r.get_den());
}

Rational Rational::substract(const Rational& r) const{ // Método que devuelve la resta de b-a en forma de clase Rational
  return add(r.opposite());
}

Rational Rational::multiply(const Rational& r) const{ // Método que devuelve la multiplicación de a * b en forma de clase Rational
  return Rational(get_num() * r.get_num(), get_den() * r.get_den());
}

Rational Rational::divide(const Rational& r) const{ // Método que devuelve la división entre a y b en forma de clase Rational
